...
private:
  ...
  //   We can't store this object in each other as hierarchy requires as there's a cycle.
  //   Have to store a pointer. We can use unique_ptr.
...
class Declarations {
  ...
  public:
    ...
    // If we copy construct, these are just copies,
    //   it's fine to take them over.
    Declarations(std::vector<ConstDef> const_defs,
                 std::vector<TypeDef> type_defs, std::vector<VarDecl> var_decls,
